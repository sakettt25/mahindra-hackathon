...
import { encodeStringToFrames } from "@/lib/qr-protocol";
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-border bg-card p-5 text-center">
                ...
                {offer ? (
                  <div className="flex flex-col items-center gap-3">
                    {showQr === "offer" ? (
                      <>
                        <QrBroadcaster frames={encodeStringToFrames(offer)} />
                        <button
                          onClick={() => setShowQr(null)}
                          className="mt-2 text-xs mono text-muted-foreground hover:text-foreground underline"
                        >
                          Hide QR / Show Copy Button
                        </button>
                      </>
                    ) : (
                      <>
                        <div className="flex gap-2 w-full justify-center">
                          <button
                            ...
                            className="flex items-center gap-2 rounded-lg bg-signal px-4 py-2 mono text-sm text-primary-foreground hover:bg-signal/90 transition-colors"
                          >
                            <Copy className="h-4 w-4" /> Copy Offer
                          </button>
                          <button
                            onClick={() => setShowQr("offer")}
                            className="flex items-center gap-2 rounded-lg bg-card border border-signal text-signal px-4 py-2 mono text-sm hover:bg-signal/10 transition-colors"
                          >
                            <QrCode className="h-4 w-4" /> Show as QR
                          </button>
                        </div>
                        <p className="text-xs text-muted-foreground mt-2">
                          Send this to the joining device via any local method (AirDrop, Bluetooth, or camera scan)
                        </p>
                      </>
                    )}
                  </div>
              ...
              <div className="rounded-2xl border border-border bg-card p-5">
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Once the other device generates an answer, scan their QR code or paste it here.
                </p>
                {isScanning ? (
                  <div className="flex flex-col items-center gap-4">
                    <QrScanner 
                      onRawString={(raw) => {
                        setIsScanning(false);
                        setRemoteInput(raw);
                        handleProcessAnswer(raw);
                      }} 
                    />
                    <button onClick={() => setIsScanning(false)} className="text-xs mono underline">Cancel Scan</button>
                  </div>
                ) : (
              ...
              {answer && (
                <div className="rounded-2xl border border-border bg-card p-5 text-center transition-all">
                  ...
                  <div className="flex flex-col items-center gap-3">
                    {showQr === "answer" ? (
                      <>
                        <QrBroadcaster frames={encodeStringToFrames(answer)} />
                        <button
                          onClick={() => setShowQr(null)}
                          className="mt-2 text-xs mono text-muted-foreground hover:text-foreground underline"
                        >
                      ...
                      <>
                        <div className="flex gap-2 w-full justify-center">
                          <button
                            ...
                            className="flex items-center gap-2 rounded-lg bg-important px-4 py-2 mono text-sm text-primary-foreground hover:bg-important/90 transition-colors"
                          >
                            <Copy className="h-4 w-4" /> Copy Answer
                          </button>
                          <button
                            onClick={() => setShowQr("answer")}
                            className="flex items-center gap-2 rounded-lg bg-card border border-important text-important px-4 py-2 mono text-sm hover:bg-important/10 transition-colors"
                          >
                            <QrCode className="h-4 w-4" /> Show as QR
                          </button>
                        </div>
                        <p className="text-xs text-muted-foreground mt-2">
                          Show this QR code to the hosting device or copy it.
                        </p>
                      </>
                    )}
                  </div>
